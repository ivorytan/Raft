...
func TestIsNewRoute(t *testing.T) {
	// Test case 1: newId is equal to currentId, should return false
	id := ID{1, 2, 3, 4}
	newId := ID{1, 2, 3, 4}
	currentId := ID{1, 2, 3, 4}
	result := id.IsNewRoute(newId, currentId)
	if result != false {
		t.Errorf("IsNewRoute failed. Expected false, got true")
	}

	// Test case 2: newId has a longer shared prefix with id than currentId, should return true
	id = ID{1, 2, 3, 4}
	newId = ID{1, 2, 3, 5}
	currentId = ID{1, 2, 2, 6}
	result = id.IsNewRoute(newId, currentId)
	if result != true {
		t.Errorf("IsNewRoute failed. Expected true, got false")
	}

	// Test case 3: currentId has a longer shared prefix with id than newId, should return false
	id = ID{1, 2, 3, 4}
	newId = ID{1, 2, 2, 4}
	currentId = ID{1, 2, 3, 3}
	result = id.IsNewRoute(newId, currentId)
	if result != false {
		t.Errorf("IsNewRoute failed. Expected false, got true")
	}

	// Test case 4: newId and currentId have the same shared prefix length with id, but newId has a better next digit
	id = ID{1, 2, 3, 4}
	newId = ID{1, 2, 3, 5}
	currentId = ID{1, 2, 3, 6}
	result = id.IsNewRoute(newId, currentId)
	if result != true {
		t.Errorf("IsNewRoute failed. Expected true, got false")
	}

	// Test case 5: newId and currentId have the same shared prefix length with id, but currentId has a better next digit
	id = ID{1, 2, 3, 4}
	newId = ID{1, 2, 3, 3}
	currentId = ID{1, 2, 3, 2}
	result = id.IsNewRoute(newId, currentId)
	if result != false {
		t.Errorf("IsNewRoute failed. Expected false, got true")
	}

	// Test case 6: newId and currentId have the same shared prefix length with id, and the same next digit, should return false
	id = ID{1, 2, 3, 4}
	newId = ID{1, 2, 3, 5}
	currentId = ID{1, 2, 3, 5}
	result = id.IsNewRoute(newId, currentId)
	if result != false {
		t.Errorf("IsNewRoute failed. Expected false, got true")
	}
